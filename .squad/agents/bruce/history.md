...
### Stable ID Implementation (from coordinator)
- **Coordinator Decision:** Changed ResistorReading.Id from Guid to deterministic string
- **Formula:** `det_{centerX}_{centerY}` with 10-unit grid quantization in 0-1000 virtual coordinate space
- **Applied to:** ResistorDetectionService, ResistorOverlayDrawable, all Dictionary/HashSet/List types
- **Commit:** 18af448 on squad/8-resistor-detection-service
- **Lesson:** Random IDs per frame break UI hysteresis tracking; spatial quantization enables stable frame-to-frame identification
- **Impact:** Rhodes was able to approve PR #46 only after this change — random Guids violated hysteresis requirements

### Pause/Resume Interface Enhancement (from coordinator)
- **Addition:** IResistorDetectionService now includes void Pause() and void Resume() method signatures
- **Motivation:** Battery optimization requires pausing ML inference during Window.Stopped (App.xaml.cs lifecycle)
- **Implementation Context:** ResistorDetectionService uses volatile bool _isPaused for thread-safe pause without locks
- **Lesson:** Service interfaces must declare lifecycle methods before concrete implementations can use them
- **Impact:** Enables PR #53 (battery optimization) to rebase cleanly onto PR #46

### Color Band Extraction Scope (from coordinator/fix agent decision document)
- **Scope Decision:** Color band extraction intentionally excluded from PR #46
- **Rationale:** Service skeleton functional without extraction; placeholder returns empty results
- **Tracking:** Skipped test in test suite explicitly documents gap and will un-skip when extraction ships
- **Lesson:** Unfinished features can be merged if infrastructure is in place and gap is documented
- **Impact:** Merging service skeleton unblocks Shuri (mobile) and downstream integration work
- **Non-Blocking:** Latent _previousDetections accumulation bug will be addressed when extraction ships
